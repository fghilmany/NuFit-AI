...
class ExerciseLibraryRepositoryImpl(
    ...
    private fun Exercise.toInsertRow(): ExerciseInsertRow = ExerciseInsertRow(
        ...
        flagPriorityJson = json.encodeToString(flagPriority),
        loadedBodyAreasJson = loadedBodyAreas?.let { json.encodeToString(it) },
        equivalentSubstitutesJson = equivalentSubstitutes?.let { json.encodeToString(it) },
        regressionChain = regressionChain,
        progressionChain = progressionChain,
        levelUpRequirement = levelUpRequirement,
        relatedMovementPatternsJson = relatedMovementPatterns?.let { json.encodeToString(it) },
    ...
    private fun ExerciseRow.toEntity(): Exercise = Exercise(
        ...
        flagPriority = json.decodeFromString<Set<ExerciseFlag>>(flag_priority),
        loadedBodyAreas = loaded_body_areas?.let { json.decodeFromString<Set<BodyArea>>(it) },
        equivalentSubstitutes = equivalent_substitutes?.let { json.decodeFromString<Map<EquipmentCategory, String>>(it) },
        regressionChain = regression_chain,
        progressionChain = progression_chain,
        levelUpRequirement = level_up_requirement,
        relatedMovementPatterns = related_movement_patterns?.let { json.decodeFromString<Set<MovementPattern>>(it) },
