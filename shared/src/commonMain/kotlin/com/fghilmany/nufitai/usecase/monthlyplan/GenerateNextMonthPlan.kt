 ...
 *
 * KNOWN GAP -- AC-5 ("kehadiran rendah -> intensity tidak dinaikkan") is only PARTIALLY
 * satisfied: neither generator currently escalates `startingLevelPerPattern` cycle-over-cycle in
 * the first place (Logged-In tier's `CalibrateStartingLevel` always falls through to its safe
 ...
 * future escalation mechanism (e.g. carrying `ProgressionGating`'s per-checkpoint level-ups
 ...
 * prior cycle's `startingLevelPerPattern` forward -- a full re-assessment-driven regeneration
