...
class BuildMovementPoolTest {
    ...
    fun `given bodyweight-only selection when invoked then dumbbell exercises are excluded but bodyweight always included`() {
        ...
        val pool = BuildMovementPool(exercises, equipmentPreference = emptySet(), level = Level.BEGINNER)
    ...
    fun `given beginner level when invoked then barbell is deferred out of the pool entirely`() {
        ...
        val pool = BuildMovementPool(exercises, equipmentPreference = setOf(EquipmentCategory.BARBELL), level = Level.BEGINNER)
    ...
    fun `given intermediate level when invoked then barbell is included`() {
        ...
        val pool = BuildMovementPool(exercises, equipmentPreference = setOf(EquipmentCategory.BARBELL), level = Level.INTERMEDIATE)

        assertTrue(pool[MovementPattern.SQUAT]?.map { it.id }?.contains("BB-SQUAT-000") == true)
    }

    @Test
    fun `given mixed levels when invoked then each pattern group is sorted regression before standard before progression`() {
        val exercises = listOf(
            testExercise("BW-SQUAT-PROG", level = ExerciseLevel.PROGRESSION),
            testExercise("BW-SQUAT-REG", level = ExerciseLevel.REGRESSION),
            testExercise("BW-SQUAT-STD", level = ExerciseLevel.STANDARD),
        )
    ...
    // POOL-04, DoD: sparse equipment selection (Bodyweight-only) with an empty pattern -> fallback via equivalentSubstitutes.
    @Test
    fun `given bodyweight-only pool with no Pull Vertical exercises when fallbackFor invoked then finds a equivalentSubstitutes candidate`() {
        ...
        val bodyweightWithSubstitute = testExercise(
            ...
            equivalentSubstitutes = mapOf(EquipmentCategory.PULL_UP_BAR to "PUB-PULLV-000"),
        )
        val allExercises = listOf(pullUpBarExercise, bodyweightWithSubstitute)

        val fallback = BuildMovementPool.fallbackFor(MovementPattern.PULL_VERTICAL, allExercises, excludedIds = setOf("PUB-PULLV-000"))
    ...
    fun `given a pattern with truly no exercises anywhere when fallbackFor invoked then returns null`() {
        val fallback = BuildMovementPool.fallbackFor(MovementPattern.PULL_VERTICAL, allExercises = emptyList(), excludedIds = emptySet())
